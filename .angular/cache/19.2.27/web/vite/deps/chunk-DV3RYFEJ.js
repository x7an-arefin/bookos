...
var Fragment = class _Fragment {
  ...
  */
  child(index2) {
    ...
    if (!found2) throw new RangeError("Index " + index2 + " out of range for " + this);
    return found2;
  }
  /**
...
function removeRange(content, from, to) {
  let {
    index: index2,
    offset
  } = content.findIndex(from), child = content.maybeChild(index2);
  let {
    index: indexTo,
    offset: offsetTo
  ...
  if (index2 != indexTo) throw new RangeError("Removing non-flat range");
...
function insertInto(content, dist, insert, openStart, openEnd, parent) {
  let {
    index: index2,
    offset
  } = content.findIndex(dist), child = content.maybeChild(index2);
  if (offset == dist || child.isText) {
    if (parent && openStart <= 0 && openEnd <= 0 && !parent.canReplace(index2, index2, insert)) return null;
...
var ResolvedPos = class _ResolvedPos {
  ...
  */
  get nodeAfter() {
    ...
    if (index2 == parent.childCount) return null;
  ...
  */
  get nodeBefore() {
    ...
    if (dOff) return this.parent.child(index2).cut(0, dOff);
  ...
  */
  posAtIndex(index2, depth) {
    ...
    for (let i = 0; i < index2; i++) pos += node.child(i).nodeSize;
    return pos;
  }
  /**
  ...
  */
  marks() {
    ...
    if (this.textOffset) return parent.child(index2).marks;
...
var Node = class _Node {
  ...
  */
  childAfter(pos) {
    ...
    return {
      node: this.content.maybeChild(index2),
      index: index2,
      offset
    };
  }
  ...
  */
  childBefore(pos) {
    ...
    if (offset < pos) return {
      node: this.content.child(index2),
      index: index2,
      offset
    };
...
var StepMap = class _StepMap {
  ...
  */
  recover(value) {
    ...
    if (!this.inverted) for (let i = 0; i < index2; i++) diff += this.ranges[i * 3 + 2] - this.ranges[i * 3 + 1];
  ...
  */
  touches(pos, recover) {
    ...
    for (let i = 0; i < this.ranges.length; i += 3) {
      ...
      if (pos <= end && i == index2 * 3) return true;
...
function liftTarget(range) {
  ...
  for (let depth = range.depth, contentBefore = 0, contentAfter = 0; ; --depth) {
    ...
    if (depth < range.depth && node.canReplace(index2, endIndex, content)) return depth;
    if (depth == 0 || node.type.spec.isolating || !canCut(node, index2, endIndex)) break;
    if (index2) contentBefore = 1;
...
function canSplit(doc3, pos, depth = 1, typesAfter) {
  ...
  for (let d = $pos.depth - 1, i = depth - 2; d > base2; d--, i--) {
    ...
    if (!node.canReplace(index3 + 1, node.childCount) || !after.type.validContent(rest)) return false;
  }
...
function joinPoint(doc3, pos, dir = -1) {
  ...
  for (let d = $pos.depth; ; d--) {
    ...
    if (before && !before.isTextblock && joinable2(before, after) && $pos.node(d).canReplace(index2, index2 + 1)) return pos;
    if (d == 0) break;
...
function insertPoint(doc3, pos, nodeType) {
  ...
  if ($pos.parentOffset == 0) for (let d = $pos.depth - 1; d >= 0; d--) {
    ...
    if ($pos.node(d).canReplaceWith(index2, index2, nodeType)) return $pos.before(d + 1);
    if (index2 > 0) return null;
  }
  if ($pos.parentOffset == $pos.parent.content.size) for (let d = $pos.depth - 1; d >= 0; d--) {
    let index2 = $pos.indexAfter(d);
    if ($pos.node(d).canReplaceWith(index2, index2, nodeType)) return $pos.after(d + 1);
    if (index2 < $pos.node(d).childCount) return null;
  }
  return null;
}
...
function contentAfterFits($to, depth, type, match, open) {
  ...
  if (index2 == node.childCount && !type.compatibleContent(node.type)) return null;
...
function replaceRange(tr, from, to, slice) {
  ...
  for (let j = slice.openStart; j >= 0; j--) {
    ...
    for (let i = 0; i < targetDepths.length; i++) {
      ...
      if (parent.canReplaceWith(index2, index2, insert.type, insert.marks)) return tr.replace($from.before(targetDepth), expand ? $to.after(targetDepth) : to, new Slice(closeFragment(slice.content, 0, slice.openStart, openDepth), openDepth, slice.openEnd));
    }
  }
  ...
  updateNextNode(node, outerDeco, innerDeco, view, index2, pos) {
    for (let i = this.index; i < this.top.children.length; i++) {
      ...
      if (next instanceof NodeViewDesc) {
        ...
        if (preMatch2 != null && preMatch2 != index2) return false;
...
function defaultTripleClick(view, inside, event) {
  ...
  if (selection instanceof TextSelection && doc3.eq(view.state.doc)) view.input.mouseDown = new TripleClickDrag(view, selection);
  return true;
}
function selectionForTripleClick(view, inside, selectNodes) {
  let doc3 = view.state.doc;
  if (inside == -1) return doc3.inlineContent ? TextSelection.create(doc3, 0, doc3.content.size) : null;
  let $pos = doc3.resolve(inside);
  for (let i = $pos.depth + 1; i > 0; i--) {
    let node = i > $pos.depth ? $pos.nodeAfter : $pos.node(i);
    let nodePos = $pos.before(i);
    if (node.inlineContent) return TextSelection.create(doc3, nodePos + 1, nodePos + 1 + node.content.size);
    else if (selectNodes && NodeSelection.isSelectable(node)) return NodeSelection.create(doc3, nodePos);
  }
  return null;
}
...
var TripleClickDrag = class extends MouseDown {
  ...
  move(event) {
    ...
    let {
      doc: doc3
    } = this.view.state, start = this.startSelection;
...
function joinMaybeClear(state, $pos, dispatch) {
  ...
  if (!$pos.parent.canReplace(index2, index2 + 1) || !(after.isTextblock || canJoin(state.doc, $pos.pos))) return false;
...
function chainCommands(...commands2) {
  return function(state, dispatch, view) {
    for (let i = 0; i < commands2.length; i++) if (commands2[i](state, dispatch, view)) return true;
    return false;
  };
}
    ...
    const schema = cleanUpSchemaItem(__spreadProps(__spreadValues({}, extraNodeFields), {
      ...
      attrs: Object.fromEntries(extensionAttributes.map((extensionAttribute) => {
        ...
        return [extensionAttribute.name, {
          default: (_a2 = extensionAttribute === null || extensionAttribute === void 0 ? void 0 : extensionAttribute.attribute) === null || _a2 === void 0 ? void 0 : _a2.default
        }];
      }))
    }));
...
var clearContent = (emitUpdate = false) => ({
  commands: commands2
}) => {
...
var forEach = (items, fn) => (props) => {
  return items.every((item, index2) => fn(item, __spreadProps(__spreadValues({}, props), {
    index: index2
  })));
};
...
var setContent = (content, emitUpdate = false, parseOptions = {}, options = {}) => ({
  editor,
  tr,
  dispatch,
  commands: commands2
...
var Keymap = Extension.create({
  ...
  addKeyboardShortcuts() {
    ...
    const handleDelete = () => this.editor.commands.first(({
      commands: commands2
    }) => [() => commands2.deleteSelection(), () => commands2.deleteCurrentNode(), () => commands2.joinForward(), () => commands2.selectNodeForward()]);
    const handleEnter = () => this.editor.commands.first(({
      commands: commands2
    }) => [() => commands2.newlineInCode(), () => commands2.createParagraphNear(), () => commands2.liftEmptyBlock(), () => commands2.splitBlock()]);
    const baseKeymap = {
      Enter: handleEnter,
...
//# sourceMappingURL=chunk-DV3RYFEJ.js.map
