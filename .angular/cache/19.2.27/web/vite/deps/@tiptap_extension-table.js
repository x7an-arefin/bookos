...
function selectedRect(state) {
  ...
  const tableStart = $pos.start(-1);
  const map = TableMap.get(table);
  return __spreadProps(__spreadValues({}, sel instanceof CellSelection ? map.rectBetween(sel.$anchorCell.pos - tableStart, sel.$headCell.pos - tableStart) : map.findCell($pos.pos - tableStart)), {
    tableStart,
    map,
    table
  });
...
function removeRow(tr, {
  ...
  for (let col = 0, index = row * map.width; col < map.width; col++, index++) {
    ...
    if (row > 0 && pos == map.map[index - map.width]) {
      ...
      const cell = table.nodeAt(pos);
      const attrs = cell.attrs;
      const copy = cell.type.create(__spreadProps(__spreadValues({}, attrs), {
  ...
  return function(state, dispatch) {
    ...
    if (dispatch) {
      ...
      if (state.selection instanceof CellSelection) state.selection.forEachCell((node, pos) => {
        if (node.attrs[name] !== value) tr.setNodeMarkup(pos, null, __spreadProps(__spreadValues({}, node.attrs), {
          [name]: value
        }));
      });
      else tr.setNodeMarkup($cell.pos, null, __spreadProps(__spreadValues({}, $cell.nodeAfter.attrs), {
        [name]: value
      }));
      dispatch(tr);
    }
...
function isolateHorizontal(tr, map, table, start, left, right, top, mapFrom) {
  ...
  for (let col = left; col < right; col++) {
    ...
    if (map.map[index - map.width] == pos) {
      ...
      tr.insert(tr.mapping.slice(mapFrom).map(map.positionAt(top, cellLeft, table)), cell.type.createAndFill(__spreadProps(__spreadValues({}, cell.attrs), {
...
function updateColumnWidth(view, cell, width) {
  ...
  for (let row = 0; row < map.height; row++) {
    ...
    tr.setNodeMarkup(start + pos, null, __spreadProps(__spreadValues({}, attrs), {
      colwidth
    }));
  }
...
function handleDecorations(state, cell) {
  ...
  for (let row = 0; row < map.height; row++) {
    ...
    if ((col == map.width - 1 || map.map[index] != map.map[index + 1]) && (row == 0 || map.map[index] != map.map[index - map.width])) {
      ...
      if ((_columnResizingPlugin = columnResizingPluginKey.getState(state)) === null || _columnResizingPlugin === void 0 ? void 0 : _columnResizingPlugin.dragging) decorations.push(Decoration.node(start + cellPos, start + cellPos + table.nodeAt(cellPos).nodeSize, {
...
var Table = Node.create({
  ...
  renderHTML({
    ...
    const {
      ...
      tableWidth,
      tableMinWidth
    } = createColGroup(node, this.options.cellMinWidth);
    const table = ["table", mergeAttributes(this.options.HTMLAttributes, HTMLAttributes, {
